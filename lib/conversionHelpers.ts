 ...
 */
export const convertToReact = <T extends Elements>(element: NodeLike<T> | ReactNode) => {
  if (isNode(element)) {
    return element;
  }
  const elementsStack: Stack<NodeLike & {
  ...
  while (!elementsStack.isEmpty()) {
    ...
    if (isGeneratedNodeLike(el)) {
      if (el.childIndex < el.children.length) {
        ...
        elementsStack.push({ ...el.children[el.childIndex], jsxChildren: [], childIndex: 0 });
        el.childIndex += 1;
      } else {
 ...
 * ```
 */
export const getTextInNode = (element: GeneratedNodeLike['children']): string => {
