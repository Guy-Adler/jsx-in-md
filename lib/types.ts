...
type Parsed<T extends Elements = Elements> = T extends Headings
    ? 'heading'
    : T extends 'ol' | 'ul'
    ? 'listItem'
    : T extends 'img'
    ? 'image'
    : T extends 'sectionContainer' | 'subsectionContainer'
    ? never
    : T;

export type GeneratedNodeLikeData<T extends Elements> = Omit<
    Parsed<T> extends never ? Record<string, never> : RootContentMap[Parsed<T>],
...
export type NodeLike<T extends Elements = Elements> =
    | GeneratedNodeLike<T>
    | JsxNodeLike;

export type AlmostFullReactNode = GeneratedNodeLike<Headings> | ReactNode;

export const isPureLeaf = <T extends Root | RootContent>(
    element: Root | RootContent,
): element is Exclude<T, { children: RootContent[] }> =>
...
export const isHeading = (
    node: Exclude<NodeLike, ReactNode>,
): node is GeneratedNodeLike<Headings> =>
    isParetNode(node) &&
    (node.nodeType === 'h1' ||
        node.nodeType === 'h2' ||
        node.nodeType === 'h3' ||
...
export const isGeneratedNodeLike = <T extends Elements, U extends NodeLike<T>>(
    element: U,
): element is Extract<U, GeneratedNodeLike<T>> =>
