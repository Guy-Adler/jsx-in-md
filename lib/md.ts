...
const mdToReactLike = (
  ...
  */
    ...
    while (!elementsStack.isEmpty()) {
        ...
        } else if (node.childIndex < node.children.length) {
...
const collapseHeaders = (
    elementsMapping: ElementsMapping,
    levelsStack: Stack<HeadingLevels | 0>,
    headingLevelsContent: Record<HeadingLevels | 0, AlmostFullReactNode[]>,
) => {
    ...
    const headingTitle = isNode(sectionHeading)
        ? sectionHeading.toString()
        : sectionHeading.children[0]?.nodeElement?.toString() ?? '';
        ...
        createElement(
            ...
            isNode(sectionHeading)
                ...
                : createElement(
                      ...
                      getElementProps(
                          sectionHeading.nodeType,
                          sectionHeading.data,
                          sectionHeading.children,
                      ), // TODO these are not valid children :(
        ...
        if (isParetNode(element!)) {
            if (isHeading(element)) {
                ...
                if (headingLevel <= currentHeadingLevel) {
                    ...
                    while (
                        !levelsStack.isEmpty() &&
                        levelsStack.peek() >= headingLevel
                    ) {
                        collapseHeaders(
                            elementsMapping,
                            levelsStack,
                            headingLevelsContent,
                        );
                    }

                    // Set the current heading level to the previous heading level.
                    currentHeadingLevel = levelsStack.isEmpty()
                        ? 0
                        : levelsStack.peek();
                }
...
const initMd = (customElements: Partial<ElementsMapping> = {}) => {
    ...
    const md = (
        ...
        return createElement(
            ...
            ...sectionizeTree(
                elementsMapping,
                mdToReactLike(elementsMapping, tree, [...jsx]),
            ),
        );
    };
